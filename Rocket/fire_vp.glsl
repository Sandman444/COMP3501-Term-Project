...
void main()
{
    ...
    float param = timer / 1.0 + phase; // The constant that divides "timer" also helps to adjust the "speed" of the fire
    ...
    float circtime = rem; // Get time value in [0..1], according to a sinusoidal wave
                                    
    // Set up parameters of the particle motion
    float t = circtime * 1.3; // Our time parameter

    // First, work in local model coordinates (do not apply any transformation)
    vec3 position = vertex;
    position += speed*normal*accel*t*t; // Particle moves up
