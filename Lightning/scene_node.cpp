...
void SceneNode::SetupShader(GLuint program){
	...
	GLint speed = glGetUniformLocation(program, "speed");
	glUniform1f(speed, phaseSpeed);

	GLint duration = glGetUniformLocation(program, "duration");
	glUniform1f(duration, phaseDuration);
}
