...
uniform vec3 front_vec = vec3(1.0, 0.0, 1.0); // right direction
float accel = 20; // An acceleration applied to the particles coming from some attraction force
float speed = 1; // Control the speed of the motion
...
void main()
{
    ...
    float circtime = 5*sin(rem); // Get time value in [0..1], according to a sinusoidal wave
    ...
    position += speed*front_vec*accel*t*t; // Particle moves right
